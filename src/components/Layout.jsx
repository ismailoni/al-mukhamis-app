...
import { LayoutDashboard, Package, Users, Wallet, LogOut, Menu, X, DollarSign, Receipt, TrendingUp, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      {/* Desktop Sidebar */}
      <aside className={`hidden md:flex ${isCollapsed ? "w-20" : "w-64"} bg-gradient-to-b from-slate-900 to-slate-800 text-white flex-col border-r border-slate-700 transition-all duration-200`}>
        <div className={`p-6 border-b border-slate-700 flex items-center ${isCollapsed ? "justify-center" : "gap-3"}`}>
          <img src={hero} alt="Al-Mukhamis" className={`${isCollapsed ? "w-10 h-10" : "w-12 h-12"} object-contain`} />
          {!isCollapsed && (
            <div>
        ...
        <div className="p-4 border-t border-slate-700 space-y-2">
          <Button 
            onClick={logout} 
            variant="ghost"
            className={`w-full ${isCollapsed ? "justify-center" : "justify-start"} text-red-400 hover:text-red-300 hover:bg-red-950/30`}
          >
          ...
          <Button
            ...
            variant="outline"
            className={`w-full border-slate-700 text-slate-200 bg-slate-800/60 hover:bg-slate-700/60 ${isCollapsed ? "justify-center" : "justify-between"}`}
          >
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 bg-slate-900 text-white p-4 flex items-center justify-between z-50 border-b border-slate-700">
        <div className="flex items-center gap-2">
          <img src={hero} alt="Al-Mukhamis" className="w-10 h-10 object-contain" />
